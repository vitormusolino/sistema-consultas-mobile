...
export default function Home({ navigation }: any) {
    ...
    async function handleLogout() {
        console.log("handleLogout chamado"); // <- log de teste

        if (Platform.OS === "web") {
            // No navegador, Alert.alert não funciona - usamos window.confirm
            const confirmou = window.confirm("Deseja realmente sair da sua conta?");
            if (confirmou) {
                console.log("Confirmado - chamando logout()");
                await logout();
        ...
        // Em dispositivo nativo (iOS/Android), mantém o Alert normal
        Alert.alert(
            "Sair",
